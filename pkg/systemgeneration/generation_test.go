	...
	for i := 4; i < 5; i++ {
		...
		for _, star := range gen.System.Stars {
			for _, orb := range star.orbitDistances {
				...
				if gg, ok := star.orbit[orb].(*ggiant); ok == true {
					uwpS := fmt.Sprintf("%v", gg.Describe())
					fmt.Println("Gas Gigant", uwpS)
					for _, moon := range gg.moons {
						uwpS := fmt.Sprintf("X%v%v%v000-0", moon.sizeCode, moon.atmoCode, moon.hydrCode)
						fmt.Println("moon", uwpS)
						details := planetarydetails.NewPlanetaryDetails(gen.Dice, moon.ExportDetails(), gen.System.Stars[0])
						details.String()
...
func TestTables(t *testing.T) {
	...
	for k, v := range starMap {
		if v.star != k {
			t.Errorf("k != v (%v %v)", v.star, k)
		}
		if v.temperature < 1 {
			t.Errorf("v < 0 (%v %v)", v.temperature, k)
		}
		if v.mass < 0 && v.mass != -999 {
			t.Errorf("v= %v (%v)", v.mass, k)
		}
		if v.luminocity < 0 && v.luminocity != -999 {
			t.Errorf("v= %v (%v)", v.luminocity, k)
		}
		if v.innerLimit < 0 && v.innerLimit != -999 {
			t.Errorf("v= %v (%v)", v.innerLimit, k)
		}
		if lowest >= v.innerLimit {
			lowest = v.innerLimit
			fmt.Println(v.star)
		}
		if v.habitableLow < 0 && v.habitableLow != -999 {
			t.Errorf("v= %v (%v)", v.habitableLow, k)
		}
		if v.habitableHigh < 0 && v.habitableHigh != -999 {
			t.Errorf("v= %v (%v)", v.habitableHigh, k)
		}
		if v.snowLine < 0 && v.snowLine != -999 {
			t.Errorf("v= %v (%v)", v.snowLine, k)
		}

		if v.outerLimit < 0 && v.outerLimit != -999 {
			t.Errorf("v= %v (%v)", v.outerLimit, k)
		}
