...
func TestSizeRelated(t *testing.T) {
	...
	for i := 1; i <= 1; i++ {
		...
		sd := NewPlanetaryDetails(dp, pl, Primary)
...
func starZonesAll() [][]float64 {
	zn := [][]float64{}
	for _, star := range stars() {
		zn = append(zn, []float64{
			getTableValues(star).innerLimit,
			getTableValues(star).habitableLow,
			getTableValues(star).habitableHigh,
			getTableValues(star).snowLine,
			getTableValues(star).outerLimit,
		})
	}
	return zn
}
