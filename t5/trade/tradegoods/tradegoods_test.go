...
func TestTradeGoodsTypes(t *testing.T) {
	for code := -1; code < 16; code++ {
		...
		if tgType == nil {
			...
			t.Errorf("trade good type was not created with code %v", code)
		}
		if tgType.class == "" {
			t.Errorf("tgType.class expected to be filled with code %v", code)
		}
		if tgType.description == "" {
			t.Errorf("tgType.description expected to be filled with code %v", code)
		}
	}
}
func TestTradeGoods(t *testing.T) {
	//var world World
	for _, data := range []string{
		...
		fmt.Println(tg)
		if tg.name == "" {
			t.Errorf("tg.name not filled")
		}
		if tg.description == "" {
			t.Errorf("tg.description not filled")
		}
	}
}
